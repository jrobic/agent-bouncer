...
const EXPECTED_COMMAND_DIGEST: readonly string[] = [
  ...
  '3 device-redirect-tee \\btee\\s+(?:-\\S+\\s+)*\\/dev\\/(sda|sdb|disk|nvme|hd|md|loop)\\w* ',
  '4 chmod-root \\bchmod\\s+(?:-\\S+\\s+)*-(?:-recursive|\\w*R\\w*)\\s+(?:-\\S+\\s+)*0?[0-7]{1,4}\\s+\\/(?:\\s|$) ',
  '5 chown-root \\bchown\\s+(?:-\\S+\\s+)*-(?:-recursive|\\w*R\\w*)\\s+(?:-\\S+\\s+)*\\S+\\s+\\/(?:\\s|$) ',
  ...
  '11 etc-write-tee \\btee\\s+(?:-\\S+\\s+)*\\/etc\\/(sudoers|passwd|shadow|hosts|ssh\\/sshd_config)\\b ',
  '12 kill-init \\bkill(?:all)?\\s+(?:(?:-s\\s+\\S+|--signal(?:=|\\s+)\\S+|-[0-9]+|-[A-Z]+[0-9]*|--)\\s+)*(?:1|init|-1)\\b ',
